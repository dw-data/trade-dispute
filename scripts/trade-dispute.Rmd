...
knitr::opts_knit$set(root.dir = "/Users/schachtk/Documents/DW/184_194_trade-dispute/")
...
- The real winners of the US-China trade dispute: [English](https://p.dw.com/p/3kXLJ) | [German](https://p.dw.com/p/3kaRJ) | [Spanish](https://p.dw.com/p/3kaRJ)
- How has the trade dispute between the United States and China affected Mexico?: [Spanish](https://p.dw.com/p/3kWmN)
- Video explainer: The real winners (and losers) of the US-China trade dispute: [English](https://www.youtube.com/watch?v=J_5u8guvAuI)

**Data analysis by:** [Kira Schacht](https://twitter.com/daten_drang)

**Reporting by:** [Kira Schacht](https://twitter.com/daten_drang), [Christine Laskowski](https://twitter.com/laskowski_c) and [Luis García Casas](https://twitter.com/ljgarciac)
...
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/185_trade-dispute_china_imports.svg", "svg",
                width_pixels=1920, height_pixels= 2160)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/185_trade-dispute_china_imports.png", "png",
                width_pixels=1920, height_pixels= 2160)
```
...
plot = ggplot(x %>% filter(short.name != "monitors"),
  ...
  labs(x = "billion USD",y="",title = "Trade dispute affected all\nof US' biggest Chinese imports",
       caption = "Change in most common US imports from China, in USD bn") +
  theme_dw() 
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/184_trade-dispute_china_products.svg", "svg",
                width_pixels=1920, height_pixels= 2700)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/184_trade-dispute_china_products.png", "png",
                width_pixels=1920, height_pixels= 2700)
```
...
```{r fig.show='hide'}
x = x %>%
  mutate(country = gsub("n Federation","",country) %>%
           gsub("Korea, Republic of","South Korea",.) %>% 
           gsub("Viet Nam","Vietnam",.) %>% 
           gsub("Taipei, Chinese","Taiwan",.),
         country = factor(country, levels = country %>% rev))
plot = ggplot(x, aes(change.imports.bn, country)) +
...
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/186_trade-dispute_winners_gains.svg", "svg",
                width_pixels=1920, height_pixels= 2700)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/186_trade-dispute_winners_gains.png", "png",
                width_pixels=1920, height_pixels= 2700)
```
...
plot = ggplot(x, aes(quarter, share, group = country, colour = country)) +
  ...
  labs(title = "Southeast Asian countries have held on\nto gains during the pandemic",
...
plot = plot + theme_dw() +
  ...
  scale_x_discrete(labels = ifelse(grepl("Q1",unique(x$quarter)), substr(unique(x$quarter), 1, 4), ""))
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/189_trade-dispute_marketshare_winners.svg", "svg",
                width_pixels=1920, height_pixels= 2160)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/189_trade-dispute_marketshare_winners.png", "png",
                width_pixels=1920, height_pixels= 2160)
```
...
plot = ggplot(x %>% filter(country == "Mexico"), aes(quarter, share, group = 0)) +
    ...
    theme_dw()
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/188_trade-dispute_marketshare_mexico.svg", "svg",
                width_pixels=1920, height_pixels= 2160)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/188_trade-dispute_marketshare_mexico.png", "png",
                width_pixels=1920, height_pixels= 2160)
```
...
```{r fig.show='hide'}
x = d_usa %>% filter(p.code %in% c(8471,8473), country %in% c("China","Mexico","Taipei, Chinese")) %>% 
  left_join(d_usa_total, by = c("p.code","quarter"), suffix = c(".country",".total")) %>% 
  left_join(p[-2]) %>% group_by(country, quarter, p.code.clean, short.name) %>%
  summarise(value.country = sum(value.country), value.total = sum(value.total)) %>% 
  mutate(share = value.country / value.total) %>% select(-value.country, -value.total) %>% 
  ungroup() %>% 
  mutate(country = ifelse(country == "Taipei, Chinese", "Taiwan", country) %>% 
...
plot = ggplot(x, aes(quarter, share, fill = country, group = country)) +
  ...
  labs(title = "Taiwan and Mexico challenge\nChina's computer dominance",
       caption = "Market share in all US imports of computers") +
  theme_dw() + theme(panel.grid = element_blank())
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/187_trade-dispute_marketshare_computers.svg", "svg",
                width_pixels=1920, height_pixels= 2160)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/187_trade-dispute_marketshare_computers.png", "png",
                width_pixels=1920, height_pixels= 2160)
```
...
plot = ggplot(x, aes(y = change, x = country, size = `2019-Q4`, fill = col)) +
  ...
  labs(title = "Electronics exports from Vietnam and Taiwan boom",
...
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/190_trade-dispute_product_bubbles.svg", "svg",
                width_pixels=1920, height_pixels= 2700)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/190_trade-dispute_product_bubbles.png", "png",
                width_pixels=1920, height_pixels= 2700)
```
...
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/194_trade-dispute_product_bubbles_mexico.svg", "svg",
                width_pixels=1920, height_pixels= 2400)
finalise_dwplot(plot,"Source: ITC Trade Map | Methodology: github.com/dw-data/trade-dispute", "graphics/preliminary/194_trade-dispute_product_bubbles_mexico.png", "png",
                width_pixels=1920, height_pixels= 2400)
```
